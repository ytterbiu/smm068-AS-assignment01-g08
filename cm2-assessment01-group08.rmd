...
extends this analysis to a broader range of thresholds. 
...
(any loss < -5.77%), and despite returns from both assets having the same
variance, Asset 1 presents a greater danger of catastrophic loss.
...
This subsection within the appendix documents how our Excel workbook is
organised and how key quantities are computed. Here we outline the content of 
each worksheet and describe the methodologies employed.

### About sheet: workbook guide and navigation {#app:about}

```{r excel-about, echo=FALSE, fig.cap="The information users are greeted with when opening the spreadsheet. The sheet is split into three parts: (1) Key information, (2) a workbook map with hyperlinks, and (3) version control.", out.width="80%"}
...
```{r excel-q1, echo=FALSE, fig.cap="Components of sheet Q1.", out.width="80%"}
...
using the coefficient $\gamma$ (gamma):
...
By calculating the difference in expected utility $(\mathbb{E}\big[U(R_2)\big] -
\mathbb{E}\big[U(R_1)\big])$ continuously across a spectrum of risk aversion, we
can see exactly how the preference for positive skewness compounds as $\gamma$
increases.
...
ggplot(plot_data, aes(x = Gamma, y = Diff)) +
  ...
  labs(
    ...
    y = expression(paste("Utility advantage (", E[U(R[2])] - E[U(R[1])], ")"))
  ) +
  theme_bw()
```
