...
```{r Q3, echo=knitr::is_html_output()}
# R code goes here
```

Consider 2 assets with random returns. Asset 1 has a Normally distributed return
with mean 6% and variance 2.44%, i.e.

$$
R_{1} \sim \mathcal{N}(\mu=0.06, \sigma^{2}=0.0244)
$$

Asset 2 has a random return $R_{2}$ where

$$
R_{2}
\sim
\begin{cases}
\mathcal{N}(\mu=0, \sigma^{2}=0.1^{2}) \quad &\text{w.p. } 0.8\\
...
## Calculate $\mathbb{E}R_{2}$ and $\mathrm{Var}R_{2}$. [5 marks]

\begin{align*} \mathbb{E}[R_{2}] & = (0\times 0.8) + (0.3\times 0.2) \\ & =
\underline{0.06} \end{align*}

Using law of total variance

$$
R_{2} \sim \begin{cases}
X\mid Z =1  &\sim \mathcal{N}(\mu=0, \sigma^{2}=0.1^{2}) \quad \text{w.p. } 0.8\\
X\mid Z =2 &\sim\mathcal{N}(\mu=0.3, \sigma^{2}=0.1^{2})  \;\text{w.p. } 0.2
\end{cases}
$$

\begin{align*} \text{Var}(R_{2}) & = \mathbb{E}[\text{Var}(X\mid Z)]
+\text{Var}(\mathbb{E}[X\mid Z]) \\ \\ \mathbb{E}[\text{Var}(X\mid Z)] & = p_{1}
\sigma_{1}^{2} + p_{2} \sigma_{2}^{2} \\ & = (0.8)(0.1) + (0.2)(0.1) \\ & = 0.1
\\ \text{Var}(\mathbb{E}[X\mid Z]) & = p(1-p)(\mu_{1}-\mu_{2})^{2} \\ & =
(0.8)(0.2)(-0.3)^{2} \\ & = 0.0144 \\ \\ \to \text{Var}(R_{2}) & = 0.1 + 0.0144
\\ & = \underline{0.1144} \end{align*}

## Use **R** to estimate $\mathbb{E}R_{2}$ and $\mathrm{Var}R_{2}$. [5 marks]
...
## Repeat part Q3c above for asset 2. [4 marks]

## Use **R** to plot the probability density functions of returns for assets 1 and 2 on the same axes. [3 marks]

## Use **R** to plot the cumulative density functions of returns for assets 1 and 2 on the same axes. [2 marks]

## Based on your answers to parts Q3a–Q3f above, comment on the risk involved in asset 2, compared to asset 1. [4 marks]
