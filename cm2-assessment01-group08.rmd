...
described in this report to be applied quickly to other stock combinations.
...
Future development may include expanding the universe of available assets and
allowing users to input dynamic Yahoo Finance tickers, though this would
require optimising the data-fetching architecture to avoid API rate limits. 
While the present implementation strictly mirrors the project’s core analysis 
(a 5-asset portfolio with a no-short-selling constraint), there is also scope to
extend this to incorporate flexible portfolio sizes and allow for short selling 
(relaxing the constraint such that weights can be negative). 

Future enhancements could integrate the Monte Carlo simulations and exact
numerical integration for utility functions developed elsewhere in this project,
and allow users to select different aggregation frequencies (for example, weekly
or monthly returns) within the app.
