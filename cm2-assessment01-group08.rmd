...
We attempt to find portfolio weights $\mathbf{w}$ that minimise the portfolio variance while yielding the target mean return, i.e. a solution to the following optimisation problem:

$$
\begin{align*}
\min &\quad \mathbf{w}^T \mathbf{D} \mathbf{w} \\
\text{s.t.} &\quad \mathbf{1}^T \mathbf{w} = 1 \\
  &\quad \mathbf{\bar{R}}^T \mathbf{w} = E \\
...
$$
\begin{align*}
\min &\quad \frac{1}{2}\mathbf{w}^T \mathbf{D} \mathbf{w} - \mathbf{d}^T \mathbf{w} \\
\text{s.t.} &\quad \mathbf{A}^T \mathbf{b} \geq \mathbf{b_0} 
\end{align*}
$$
