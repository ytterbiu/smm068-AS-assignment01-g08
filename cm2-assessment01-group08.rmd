...
_typical_ risk-averse investors[^utility-theory] and it provides a lower risk
...
```{r Q3-final-combined-plot, eval=TRUE, echo=FALSE, fig.width=7, fig.height=6.0, fig.cap="Top: PDFs. Middle: CDFs with crossover points (equal shortfall probabilities). Bottom: CDF difference (R2 - R1); positive means higher shortfall probability for R2 at that x.", fig.pos='!ht', message=FALSE, warning=FALSE}
x_vals <- seq(-0.6, 0.6, by = 0.001)
...
See \S\@ref(apd-utility-theory) for a full evaluation of both assets using utility theory.

Whilst the cumulative distribution function makes sense statistically, it can be
confusing from a finance perspective. Instead we can look at the survival
functions (which is the inverse of the previous figure). In the survival
function a larger value is 'better' or the probability that the returns are
...
While both assets share identical Mean and Variance, their risk profiles differ
significantly in the tails. Asset 1 carries higher tail risk (higher
