...
& \mathbb{P}(R_1 < L)
= \displaystyle \int_{-\infty}^{L} f_{R_1}(x)\, \mathrm{d}x \\[6pt]

\hline
\end{array}

```{r}
prob_val1 <- pnorm(0, mean = 0.06, sd = 0.156205)
prob_val2 <- pnorm(-0.1, mean = 0.06, sd = 0.156205)
...
\begin{array}{ll}
\hline
\text{Answer} & \mathbf{`r round(100 * prob_val1, 3)`}\% \\
\hline
\end{array}
\]
...
\begin{array}{ll}
\hline
\text{Answer} & \mathbf{`r round(100 * prob_val2, 3)`}\% \\
\hline
\end{array}
\]
...
+ 0.2 \int_{-\infty}^{L} f_{\mathcal{N}(0.3,\,0.1^2)}(x)\, \mathrm{d}x \\
...
&= 0.8 \times \Phi\!\left( \frac{0 - 0}{0.1} \right) + 0.2 \times \Phi\!\left( \frac{0 - 0.3}{0.1} \right) \\
&= 0.8 \times \Phi\!\left(0\right) + 0.2 \times \Phi\!\left(-3\right) \\
&=`r round(0.8*0.5+0.2*pnorm(-3),5)`\\
\end{aligned}
\]
...
\begin{array}{ll}
\hline
\text{Answer} & \mathbf{`r 100*round(0.8*0.5+0.2*pnorm(-3),5)`}\% \\
\hline
\end{array}
\]
...
&= 0.8 \times \Phi\!\left( \frac{-0.1 - 0}{0.1} \right) + 0.2 \times \Phi\!\left( \frac{-0.1 - 0.3}{0.1} \right) \\
&= 0.8 \times \Phi\!\left(-1\right) + 0.2 \times \Phi\!\left(-4\right) \\
&=`r round(0.8*pnorm(-1)+0.2*pnorm(-4),5)`\\
\end{aligned}
\]
...
\begin{array}{ll}
\hline
\text{Answer} & \mathbf{`r 100*round(0.8*pnorm(-1)+0.2*pnorm(-4),5)`}\% \\
\hline
\end{array}
\]
...
The plotting range $[-0.5, 0.6]$ was selected based on definite outlier. 
For Asset1, $R_1 \sim \mathcal{N}(0.06, 0.0244)$ with standard deviation $\sigma_1 = \sqrt{0.0244} \approx 0.156205$, giving a three-sigma inverval of $0.06 \pm 3\times\sigma_1 \approx [-0.41, 0.53]$.
For asset 2, the two normal components have means 0 and 0.3 with standard deviation 0.1, resulting in three-sigma intervals of $[-0.3, 0.3]$ and $[0.0, 0.6]$ , respectively.
The selected range $[-0.5, 0.6]$ therefore covers approximately three standard deviations for both assets.
...
_[4 marks]_
 ...
 Based on the results from $Q3(a)$ to $Q3(f)$, asset 2 appears to be riskier than asset1, even though their expected returns are similar. Asset 1 follows a single normal distribution, so its returns are more stable and predictable. In contrast, asset 2 is a mixture of two normal distributions, which adds more uncertainty. The shortfall probabilities also indicate that asset 2 has a higer of risk of extreme outliers. This can be seen in the PDF, where asset 2 shows a more irregular shape and heavier tails. The CDF also indicates that asset 2 has a higer of risk of extreme outcomes. 
