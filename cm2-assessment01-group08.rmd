...
- Bs
 ...
 1. **Establish the investor's objectives and the single-period horizon**
   : Mean-variance portfolio theory is a single-step framework, so the time period and goal must be specified
   
 2. **Clarify the investor's attitude to risk (risk-averse and non-satiated)**
   : At a given level of risk they prefer higher return, and at a given return they prefer lower variance.
 
 3. **Determine the investor's optimal portfolio using indifference curves.**
   : No other portfolio offers higer return for the same variance or lower variance for the same return
 
 4. **Restrict attention to the efficient frontier only.**
   : An efficient portfolio is one for which no other portfolio offers higher return for the same variance( or lower variance for the same return)
   
 5.	**Recognise that all expected returns, variances and covariances are assumed known.**
   : Application requires estimates of expected returns and the variance-covariance structure, which may be unstable.
 
 6. **link the choice to utility / indifference curves.**
   : The optimal portfolio is obtained by selecting the efficient portfolio that maximises expected utility
  
 7.**Be explicit about constraints (eg no short-selling, fully invested)**
   : Constraints can materially alter the feasible set and may produce corner solutions on the frontier.
   
 8. **Discuss estimation error and the unreliability of historical inputs.**
   : historical estimates may not predict future behaviour. Result can be sensitive to small changes in inputs.
   
 9. **consider other important investment risks beyond variance.**
  : Inflation, currency, reinvestment, marketability and credit risks may affect suitability.
  
10. **Acknowledge limitations and the need for review**
  : Basic MPT ignores liabilities / actuarial risk : portfolios should be monitored and updated as conditions change
...
The results from part \@ref(qthreePtC) and \@ref(qthreePtD) are summarised 
in Table \@ref(tab:qthr-pt-CD-summary).
...
Table \@ref(tab:qthr-pt-CD-extension) extends this analysis to a broader range
of thresholds. This comparison reveals that while Asset 2 has a higher
probability of moderate losses (returns between roughly −5.77% and 0%), the
relationship inverts in the extreme tail.

Asset 1 carries a higher probability of severe drawdowns (e.g., returns <−15%),
...
shortfall_long %>%
  ...
  kable(
    ...
    linesep = c("", "", "", "\\addlinespace"),
    align = "llcc",
    col.names = c("Label", "Threshold Value", "R1 (\\%)", "R2 (\\%)"),
...
```{r Q3-part-f-ggplot, eval=TRUE, echo=FALSE, fig.width=7, fig.height=6.0, fig.cap="Top: PDFs. Middle: CDFs with crossover points (equal shortfall probabilities). Bottom: CDF difference (R2 - R1); positive means higher shortfall probability for R2 at that x.", fig.pos='!ht', message=FALSE, warning=FALSE}
x_vals <- seq(-0.6, 0.6, by = 0.001)
...
cross_df <- cross_df %>%
  mutate(
    y_cdf  = ifelse(row_number() %% 2 == 1, 0.85, 0.15),
...
p2 <- ggplot(df_cdf_long, aes(x = x, y = y, color = Asset, linetype = Asset)) +
  ...
  labs(title = "Cumulative distribution functions", x = NULL, y = "Probability") +
  base_theme
...
While both assets share identical Mean and Variance, their risk profiles differ in the tails. Asset 1 carries higher tail risk (higher
probability of losses below the crossover (value)
calculated in (ref), whereas Asset 2 entails a higher frequency of small losses.
A risk-averse investor should prefer Asset 2
due to its bounded downside and positive skewness, while a Mean-Variance
investor would view them as equivalent (see (appendix ref) for utility proofs).
...
[^utility-skew]: 
    ...
    \@ref(tab:exact-utility-table)), a CRRA utility function with $\gamma > 0$
    penalises the negative skewness of Asset 1, mathematically confirming it as
    the inferior choice for risk-averse investors.