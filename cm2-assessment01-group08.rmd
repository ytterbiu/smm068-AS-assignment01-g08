...
#   - rootSolve:finds roots of (fairly straightforward) equations
#   - optimx: handles non-linear unconstrained optimisation (& box-constrained)
#   - quadprog: to solve a quadratic programming problem
#   - kableExtra: for general tables
#   - DT: for HTML interactive data tables
#   - plotly: for interactive html plots
#   - pander: for writing to pandoc
#
# Acknowledgements
...
# Without this code the stocks data are downloaded fresh every time we compile
...
if (file.exists(cache_file)) {
  ...
  price_list <- lapply(mget(stocks, envir = data_env), Ad)
...
# Portfolio optimisation {#qtwo}

Note: Average daily returns were converted to annualised units and a standard 
252 trading day multiplier was chosen for consistency 
(i.e., $z_{ann} = z_{daily} \times 252$).
...
\text{Constraint 2}
  & \sum_{i=1}^{N}x_i\overline{R}_i = E \\[6pt]
\hline
\end{array}
$$


```{r Q2a, echo=FALSE, fig.pos="!ht"}
# ==============================================================================
# Q2(a) estimate all required statistics for the 5 stocks
# ==============================================================================

# Calculate discrete daily returns and drop the first NA row
df_returns <- prices %>%
  arrange(Date) %>%
...
# Estimate Standard Deviations (Volatilities)
sd_a <- sqrt(diag(cov_matrix_a))
```

## Stock statistics {#qtwoa}

Required statistics for the 5 selected stocks were calculated in preparation for 
mean-variance portfolio optimisation. Daily returns were calculated
from the adjusted closing prices. The expected returns and variance-covariance
matrix were estimated using the sample mean and covariance of these daily
returns, scaled by 252 to annualise them.

### Expected annual returns {-}

```{r, echo=FALSE}
# Using simple kable tables for these statistics.
# Annoying bug is the use of bold text in rmd breaks the monospacing. Without 
# bold text the pdf renders nicely with numbers all aligned. Assessment 
# guidelines state the answers must be in bold, so have done this below, but it 
# doesn't look as professional as it could ¯\_(ツ)_/¯. 
# 
# Opted not to combine expected annual returns and standard deviation to mirror
# format used in spreadsheet.
ret_table <- data.frame(
  Stock = names(mean_returns_a),
  `Annual Return` = sprintf("**%.2f%%**", mean_returns_a * 100),
  check.names = FALSE
)
...
kable(
  ...
  format = "markdown",
  align = c("l", "c"),
  col.names = c("Stock", "Expected returns (annualised)")
)
```
...
```{r, echo=FALSE}
cov_formatted <- as.data.frame(cov_matrix_a) %>%
  mutate(across(everything(), ~ sprintf("**%s**", format_spaced_6dp(.))))
rownames(cov_formatted) <- paste0("**", rownames(cov_matrix_a), "**")

kable(
  cov_formatted, 
  format = "markdown", 
  align = c(rep("r", ncol(cov_formatted))) 
)
```
...
sd_table <- data.frame(
  Stock = names(sd_a),
  `Annual Volatility` = sprintf("**%.2f%%**", sd_a * 100)
)
kable(
  sd_table,
  format = "markdown",
  align = c("l", "c"),
  col.names = c("Stock", "Standard deviation (annualised)")
)
```

### Correlation coefficient matrix {-}

*Note: the correlation coefficient is not used in the efficient frontier
calculation. We include it here for posterity.*

```{r, echo=FALSE}
cor_matrix_a <- cor(df_returns %>% select(-Date))

cor_formatted <- as.data.frame(cor_matrix_a) %>%
  mutate(across(everything(), ~ sprintf("**%s**", format_spaced_6dp(.))))
rownames(cor_formatted) <- paste0("**", rownames(cor_matrix_a), "**")

kable(
  cor_formatted, 
  format = "markdown", 
  align = c(rep("r", ncol(cor_formatted)))
)
```
...
## Efficient frontier formed by investing in the 5 stocks {#qtwob}

To find portfolio weights $w$ that minimise the portfolio variance while 
yielding the target mean return $E$ invovles solving the following optimisation 
problem:
<!-- prettier-ignore -->
\begin{align*}
\min &\quad {w}^T {D} {w} \\
\text{s.t.} &\quad {1}^T {w} = 1 \\
  ...
  &\quad {w} \geq {0} \quad \text{(No short selling)}
\end{align*}
...
# > Target Return = E, 
# > no short selling
Amat <- cbind(rep(1, n_sec), mean_returns_a, diag(n_sec))

# Generate expected asset returns from the minimum to maximum 
targets <- seq(min(mean_returns_a), max(mean_returns_a), length.out = 500)
...
eff_frontier <- map_dfr(targets, function(tar) {
  ...
  w <- sol$solution
  w[w < 1e-10] <- 0 # Clean up floating point inaccuracies
  w <- w / sum(w)
  
  tibble(
    Risk = sqrt(as.numeric(t(w) %*% cov_matrix_a %*% w)),
    Return = sum(w * mean_returns_a)
  )
})
...
# Get global minimal point
gmv_point <- eff_frontier[gmv_idx, ]

# Prepare individual asset data for plotting
asset_points <- tibble(
  Stock = names(mean_returns_a),
  Risk = sd_a,
  Return = mean_returns_a
)

# Plotting using ggplot2 to match Q1 aesthetic
base_theme <- theme_bw() +
  theme(
    plot.title = element_text(hjust = 0.5, face = "bold"),
    legend.position = "bottom",
...
pal <- RColorBrewer::brewer.pal(8, "Dark2")
my_cols <- setNames(pal[c(1, 2, 3, 5, 4)], sort(unique(asset_points$Stock)))

ggplot() +
  # Efficient frontier (top half)
  geom_line(data = efficient_segment, aes(x = Risk, y = Return), 
            color = "darkgreen", linewidth = 1.2) +
  # Bottom half of the parabola
  geom_line(data = inefficient_segment, aes(x = Risk, y = Return), 
            color = "grey60", linetype = "dashed", linewidth = 1) +
  # Individual assets
  geom_point(data = asset_points, aes(x = Risk, y = Return, color = Stock), size = 3) +
  geom_text(data = asset_points, aes(x = Risk, y = Return, label = Stock, color = Stock), 
            hjust = -0.3, vjust = 0.5, show.legend = FALSE) +
  annotate("segment", 
           x = gmv_point$Risk - 0.03, y = gmv_point$Return + 0.09, # line start
           xend = gmv_point$Risk - 0.001, yend = gmv_point$Return, # line end
           arrow = arrow(length = unit(0.2, "cm")), color = "black") +
  # hard coded text location
  annotate("text", 
           x = gmv_point$Risk - 0.05, y = gmv_point$Return + 0.105, 
           label = "Global minimum variance portfolio", color = "black", hjust = 0.5) +
  scale_color_manual(values = my_cols) +
  labs(
    title = "Mean-variance efficient frontier (no short selling)",
    x = "Risk (annualised standard deviation)",
    y = "Expected return (annualised)",
    color = "Individual Assets:"
  ) +
  scale_x_continuous(labels = scales::percent_format(accuracy = 1)) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
  coord_cartesian(xlim = c(0.01, 0.29), ylim = c(0.01, 0.40)) +
  base_theme
```

The mean-variance efficient frontier generated by solving this for a range of
target returns is shown in Figure \@ref(fig:Q2b). The global minimum variance
(GMV) portfolio is marked on the frontier and represents the portfolio with the
lowest possible risk given the no-short-selling constraint.

The GMV portfolio yields an expected annualised return of 
`r sprintf("%.3f%%", eff_frontier$Return[gmv_idx] * 100)`
with an annualised risk (standard deviation) of 
`r sprintf("%.3f%%", eff_frontier$Risk[gmv_idx] * 100)`.
These results are in agreement (to one decimal place) with the values calculated
in the Excel implementation, which is described in more detail in the appendix
(\S\@ref(app:excel-impl)). Minor differences in the final asset weights are not
unexpected due to the distinct numerical optimisation algorithms and convergence
tolerances utilised by R and Excel.

The optimal asset allocations required to
achieve this portfolio are detailed below:

```{r gmv-weights, echo=FALSE}
# Get the target return for the GMV portfolio
gmv_target <- targets[gmv_idx]

# re-run the optimiser for this point
b0vec_gmv <- c(1, gmv_target, rep(0, n_sec))
sol_gmv <- solve.QP(cov_matrix_a, rep(0, n_sec), Amat, b0vec_gmv, meq = 2)

w_gmv <- sol_gmv$solution
w_gmv[w_gmv < 1e-10] <- 0 # clean floating point inaccuracies
w_gmv <- w_gmv / sum(w_gmv)

# Outputtable
gmv_table <- data.frame(
  Stock = names(mean_returns_a),
  Weight = sprintf("**%.2f%%**", w_gmv * 100)
)
...
kable(
  ...
  format = "markdown",
  align = c("l", "c"),
  col.names = c("Stock", "Portfolio Weight")
)
```
...
# consistent colours + styles (matches Q3e ggplot)
pal <- RColorBrewer::brewer.pal(8, "Dark2")
...
# crossover points where CDFs are equal (diff_cdf = 0), 
# via sign changes + linear interpolation
d <- plot_df$diff_cdf
x <- plot_df$x
...
To explore this project further (and in an optimistic pursuit of extra credit)
we constructed an app to allow for an additional exploration of the work
described in \@ref(qone) & \@ref(qtwo) using different portfolios.

As R Markdown was used to write up this project, one natural approach would have
been simply to update the five stocks and render a new R Markdown document for
each portfolio. While this would work in principle, it is not fully
satisfactory: the document takes time to render and each iteration would require
manual narrative adjustments. To address this, we developed an interactive R
Shiny dashboard, hosted on https://www.shinyapps.io/, which provides a dynamic
interface for the construction of a portfolio under the framework described in
this report. The dashboard is available via:

- https://3enji.shinyapps.io/smm068-a01-g08-dashboard/

Figure \@ref(fig:appendix-dashboard-1) shows the dashboard start page. Currently
...
```{r appendix-dashboard-1, echo=FALSE, purl=FALSE, out.width="70%", fig.cap="The R Shiny dashboard start page. A selection of stocks are downloaded when the app is first loaded to avoid repeated API requests and throttling restrictions."}
knitr::include_graphics("fig/appendix-dashboard-1.png")
```

The statistics tab (not shown here) mirrors \@ref(qtwoa), showing the expected
returns, standard deviation and the covariance matrix for selected securities.

```{r appendix-dashboard-2, echo=FALSE, purl=FALSE, out.width="100%", fig.cap=" The Efficient Frontier tab in the R Shiny dashboard showing the portfolio weights associated with a given portfolio on the efficient frontier, along with the expected return and risk. The subjective risk tolerance can change using a slider from 0 to 100, updating both the metrics, portfolio weights, and the visual representation showing the location of the portfolio on the efficient frontier."}
knitr::include_graphics("fig/appendix-dashboard-2.png")
```

The interactive efficient frontier element of the app is shown in Figure \@ref(fig:appendix-dashboard-2).  The risk tolerance slider represents the
amount of risk an investor is willing to accept in nominal units of 0 to 100. 
Changing this slider automatically updates the portfolio weights, the risk and
expected return, and the location of the portfolio on the efficient frontier.

Future development of this app could expand upon the present implementation
which strictly mirrors the project’s core analysis (a 5-asset portfolio with a
no-short-selling constraint). The more alluring areas to develop are
incorporating flexible portfolio sizes, allowing for short selling in the model,
and including the option to select and use a risk-free asset in the portfolio.
Future development could also expand the universe of available assets and allow
users to input dynamic Yahoo Finance tickers, though this would require
optimising the data-fetching architecture to avoid API rate limits.
...
```{r excel-q1, echo=FALSE, fig.cap="Overview of sheet Q1.", out.width="85%", fig.pos="!ht"}
...
2. A GMV portfolio summary for each method, including 
