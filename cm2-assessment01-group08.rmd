...
data1 <- lapply(px, function(x) {
  ROC(as.numeric(x), type = "discrete") * 100   # % 단순수익률
})
...
Q2(a)
 Daily returns were caculated from the adjusted closing prices of the five stocks. The expected returns were obtained by taking the average of the daily returns and the annualised.
...
Answer: 
 ...
 AIG : `r sprintf("%.3f", mean_returns_a[1]*100)`% (3dp)  
 ALL : `r sprintf("%.3f", mean_returns_a[2]*100)`% (3dp)  
 CB  : `r sprintf("%.3f", mean_returns_a[3]*100)`% (3dp)  
 PGR : `r sprintf("%.3f", mean_returns_a[4]*100)`% (3dp)  
 TRV`: `r sprintf("%.3f", mean_returns_a[5]*100)`% (3dp) 
...
# plot
plot(sqrt(diag(cov_matrix_a)), mean_returns_a,
     pch = 1, cex = 1.2,
     xlab = "StdDev", ylab = "mean",
     main = "Mean Variance Portfolio",
     xlim = range(sqrt(diag(cov_matrix_a))) * c(0.9, 1.1),
     ylim = range(mean_returns_a) * c(0.9, 1.1))

# labels for assets
text(sqrt(diag(cov_matrix_a)), mean_returns_a, labels = names(mean_returns_a), pos = 4, cex = 0.9)
...
text(sqrt(diag(cov_matrix_a)), mean_returns_a,
